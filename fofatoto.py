...
.load-indicator{padding:7px 12px;font-size:12px;color:#94a3b8;text-align:center;border-top:1px solid #e2e8f0;background:#f8fafc;letter-spacing:.3px}
...
<div id="loadIndicator" class="load-indicator" style="display:none"></div>
</div>
</div>
...
var currentMode="instant",currentResults=[],currentColumns=[],currentView=[],renderedCount=0,CHUNK_SIZE=200,previewRendered=false,exportTaskId=null,exportPollTimer=null,progressUiMode="overlay",sortColumn=null,sortAsc=true,historyActiveIndex=-1,historyVisibleItems=[];
...
function appendNextChunk(){if(renderedCount>=currentView.length)return;var chunk=currentView.slice(renderedCount,renderedCount+CHUNK_SIZE);document.querySelector("#resultsTable tbody").insertAdjacentHTML("beforeend",buildRowsHtml(chunk,currentColumns));renderedCount+=chunk.length;updateLoadIndicator()}
function setupScrollLoader(){var box=document.getElementById("resultsTable").parentElement;if(!box||box._scrollBound)return;box._scrollBound=true;box.addEventListener("scroll",function(){if(renderedCount<currentView.length&&box.scrollTop+box.clientHeight>=box.scrollHeight-300)appendNextChunk()})}
function updateLoadIndicator(){var el=document.getElementById("loadIndicator");if(!el)return;if(renderedCount>=currentView.length||currentView.length===0){el.style.display="none";return}el.style.display="block";el.textContent="\u5df2\u52a0\u8f7d "+renderedCount.toLocaleString()+" / "+currentView.length.toLocaleString()+" \u6761 \u00b7 \u6eda\u52a8\u52a0\u8f7d\u66f4\u591a"}

...
var colWidthsRef=null;var lockColWidths=true;function toggleLockColWidths(el){lockColWidths=el.checked;colWidthsRef=null;stabilizeColumnWidths();renderVirtual()}function stabilizeColumnWidths(){var table=document.getElementById("resultsTable");var oldCg=table.querySelector("colgroup");if(!lockColWidths){if(oldCg)oldCg.remove();table.style.tableLayout="auto";return}if(colWidthsRef===currentResults&&oldCg)return;colWidthsRef=currentResults;var cols=currentColumns;if(oldCg)oldCg.remove();table.style.tableLayout="auto";if(!cols.length||!currentResults.length)return;var tbody=document.querySelector("#resultsTable tbody");tbody.innerHTML=buildRowsHtml(currentResults.slice(0,Math.min(100,currentResults.length)),cols);var firstRow=tbody.querySelector("tr");if(firstRow){var h=firstRow.offsetHeight;if(h>0)rowHeight=h;var widths=[];firstRow.querySelectorAll("td").forEach(function(td){widths.push(td.offsetWidth)});var total=0;widths.forEach(function(w){total+=w});var cg=document.createElement("colgroup");cols.forEach(function(col,i){var el=document.createElement("col");var w=widths[i]||100;el.style.width=total>0?((w/total)*100).toFixed(3)+"%":w+"px";cg.appendChild(el)});table.insertBefore(cg,table.firstChild)}table.style.tableLayout="fixed"}

...
var currentMode="instant",currentResults=[],currentColumns=[],currentView=[],renderedCount=0,CHUNK_SIZE=500,previewRendered=false,exportTaskId=null,exportPollTimer=null,progressUiMode="overlay",sortColumn=null,sortAsc=true,historyActiveIndex=-1,historyVisibleItems=[];
...
function appendNextChunk(){if(renderedCount>=currentView.length)return;var chunk=currentView.slice(renderedCount,renderedCount+CHUNK_SIZE);document.querySelector("#resultsTable tbody").insertAdjacentHTML("beforeend",buildRowsHtml(chunk,currentColumns));renderedCount+=chunk.length}
function getScrollBox(){return document.getElementById("resultsTable").parentElement}
function fillBuffer(){var box=getScrollBox();if(!box)return;var g=0;while(renderedCount<currentView.length&&box.scrollTop+box.clientHeight>=box.scrollHeight-2500&&g<30){appendNextChunk();g++}}
function setupScrollLoader(){var box=getScrollBox();if(!box||box._scrollBound)return;box._scrollBound=true;var tick=false;box.addEventListener("scroll",function(){if(tick)return;tick=true;requestAnimationFrame(function(){tick=false;fillBuffer()})})}

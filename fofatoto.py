...
.pagination-bar{display:flex;align-items:center;justify-content:center;gap:8px;padding:8px 12px;font-size:12px;color:#64748b;border-top:1px solid #e2e8f0;background:#f8fafc}
.pagination-bar button{padding:3px 10px;border:1px solid #cbd5e1;border-radius:4px;background:#fff;color:#334155;font-size:12px;cursor:pointer;transition:all .15s}
.pagination-bar button:hover:not(:disabled){background:var(--accent);color:#fff;border-color:var(--accent)}
.pagination-bar button:disabled{opacity:.4;cursor:default}
.pagination-bar .page-info{font-weight:500;color:#334155}
...
var currentMode="instant",currentResults=[],currentColumns=[],currentView=[],currentPage=1,PAGE_SIZE=200,previewRendered=false,exportTaskId=null,exportPollTimer=null,progressUiMode="overlay",sortColumn=null,sortAsc=true,historyActiveIndex=-1,historyVisibleItems=[];
...
document.addEventListener("DOMContentLoaded",function(){initFieldSelector();loadAccountInfo();setupModeTabs();setupSearchShortcut();updateHistoryCount();updateLayout();window.addEventListener("resize",updateLayout)});
...
function renderResults(data){var area=document.getElementById("resultsArea"),rows=data.results||[],table=document.getElementById("resultsTable"),empty=document.getElementById("emptyState");previewRendered=true;syncModeContent();var pickBtns='<button class="mini-btn" id="pickFilterBtn" onclick="enterPickMode(\'filter\')">不看</button><button class="mini-btn" id="pickQueryBtn" onclick="enterPickMode(\'query\')">选取查询</button>';var actions=rows.length>0?'<div class="stats-actions"><span class="preview-status" id="previewStatus"></span>'+pickBtns+'<button class="mini-btn" onclick="exportPreview(&quot;csv&quot;)">导出 CSV</button><button class="mini-btn" onclick="exportPreview(&quot;json&quot;)">JSON</button><button class="mini-btn" onclick="exportPreview(&quot;txt&quot;)">TXT</button></div>':"";document.getElementById("statsBar").innerHTML='<div class="stats-metrics"><span class="stat-item"><span class="stat-dot" style="background:var(--accent)"></span>总计: <span class="stat-value">'+(data.total||0).toLocaleString()+'</span></span><span class="stat-item"><span class="stat-dot" style="background:var(--success)"></span>独立IP: <span class="stat-value">'+(data.unique_ips||0).toLocaleString()+'</span></span><span class="stat-item"><span class="stat-dot" style="background:var(--warning)"></span>结果: <span class="stat-value">'+rows.length.toLocaleString()+'</span></span></div>'+actions;var cols=data.columns||[];if(cols.length===0&&rows.length>0)cols=Object.keys(rows[0]);currentColumns=cols;if(rows.length===0){table.style.display="none";empty.style.display="block";empty.textContent=excludedFilters.length?"所有结果已被「不看」排除，移除排除项可恢复显示。":((data.total||0)>0?"当前预览没有返回记录，可调大数量或更换字段后重试。":"没有匹配结果。");document.querySelector("#resultsTable thead").innerHTML="";document.querySelector("#resultsTable tbody").innerHTML="";var pb=document.getElementById("paginationBar");if(pb)pb.style.display="none";updateLayout();return}table.style.display="table";empty.style.display="none";var thead="";cols.forEach(function(col){var sc=sortColumn===col?" sorted":"";thead+="<th class=\""+sc+"\" onclick=\"sortBy('"+escHtml(col)+"')\">"+(sortColumn===col?(sortAsc?"▲ ":"▼ "):"")+escHtml(col)+"</th>"});document.querySelector("#resultsTable thead").innerHTML="<tr>"+thead+"</tr>";currentView=rows;var totalPages=Math.max(1,Math.ceil(rows.length/PAGE_SIZE));if(currentPage>totalPages)currentPage=totalPages;var start=(currentPage-1)*PAGE_SIZE,pageRows=rows.slice(start,start+PAGE_SIZE);renderTableBody(pageRows,cols);renderPagination(rows.length,totalPages);updateLayout()}
function renderTableBody(pageRows,cols){var tbody="";pageRows.forEach(function(row){tbody+="<tr>";cols.forEach(function(col){var val=row[col]!==undefined?row[col]:"",cls=(col==="ip"||col==="port"||col==="host")?" mono":"";if((col==="host"||col==="url")&&val){var url=val.indexOf("http")===0?val:"http://"+val;tbody+='<td class="'+cls+'"><a href="'+escHtml(url)+'" target="_blank" rel="noopener">'+escHtml(val)+"</a></td>"}else tbody+='<td class="'+cls+'" title="'+escHtml(val)+'">'+escHtml(val)+"</td>"});tbody+="</tr>"});document.querySelector("#resultsTable tbody").innerHTML=tbody}
function renderPagination(total,totalPages){var bar=document.getElementById("paginationBar");if(!bar)return;if(totalPages<=1){bar.style.display="none";return}bar.style.display="flex";bar.innerHTML='<button onclick="goToPage('+(currentPage-1)+')" '+(currentPage<=1?"disabled":"")+'>\u4e0a\u4e00\u9875</button><span class="page-info">\u7b2c '+currentPage+"/"+totalPages+" \u9875 \u00b7 \u5171 "+total.toLocaleString()+" \u6761</span>"+'<button onclick="goToPage('+(currentPage+1)+')" '+(currentPage>=totalPages?"disabled":"")+'>\u4e0b\u4e00\u9875</button>'}
function goToPage(p){var totalPages=Math.max(1,Math.ceil(currentView.length/PAGE_SIZE));if(p<1||p>totalPages)return;currentPage=p;var start=(currentPage-1)*PAGE_SIZE;renderTableBody(currentView.slice(start,start+PAGE_SIZE),currentColumns);renderPagination(currentView.length,totalPages);updateLayout()}

...
- **Web 界面** — 双击或 `-w` 启动本地 Web UI，支持即时预览、深度导出和批量模式
- **跨平台** — 支持 Windows / Linux / macOS，提供开箱即用的编译二进制
- **全量导出** — 突破 FOFA 单次 10000 条限制，通过 `before` 时间游标分批拉取全部结果
...
> `config.json` 固定放在脚本或可执行文件所在目录。未配置有效 API Key 时，Web UI 仍会启动并显示配置路径；CLI 查询会提示配置后退出。
...
```bash
./fofatoto -w
./fofatoto -w --port 8090
```

Web UI 仅监听 `127.0.0.1`，默认从 17380 开始自动探测可用端口，并自动打开系统浏览器。

Web UI 提供三种模式：

| 模式 | 说明 |
|------|------|
| 即时预览 | 快速查询并在页面表格中查看结果，支持字段选择、列排序、URL 跳转和当前预览导出 |
...
| `--fill` | 值 | `0.8` | 深度抓取完成百分比 (0.0-1.0) |
| `--full` | 标志 | 否 | 搜索全部历史数据 |
| `-v, --verbose` | 标志 | 否 | 显示详细查询信息 |
| `-w, --web` | 标志 | 否 | 启动 Web UI 模式 |
